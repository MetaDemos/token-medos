...
contract ERC20Fee is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable {
  ...
  string private constant BALANCE_EXCEED = "BALANCE_EXCEED";
  string private constant ALLOWANCE_EXCEED = "ALLOWANCE_EXCEED";

  string private constant FEE_FAILED = "FEE_FAILED";
  ...
  function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
    ...
    require(currentAllowance >= subtractedValue, ALLOWANCE_EXCEED);
    unchecked {
  ...
  function _transfer(
    ...
    if (from == address(this)) {
      require(fromBalance >= amount + _feeAmount, BALANCE_EXCEED);
    } else {
      require(fromBalance >= amount, BALANCE_EXCEED);
    }
    unchecked {
    ...
    if (fee > 0) {
      ...
      _feeAmount += fee;
      require(_proceedHookFee(fee), FEE_FAILED);
  ...
  function _burn(address account, uint256 amount) internal virtual {
    ...
    require(accountBalance >= amount, BALANCE_EXCEED);
    unchecked {
  ...
  function _spendAllowance(
    ...
    if (currentAllowance != type(uint256).max) {
      require(currentAllowance >= amount, ALLOWANCE_EXCEED);
      unchecked {
  ...
  function _calcFee(
    ...
    if (
      from != address(0) && to != address(0) && from != address(this) && to != address(this) && !_isExcludedFee[from] && !_isExcludedFee[to]
    ) {
      uint256 _fee;
      if (_isForcedFee[to]) {
        _fee = _feeToForced;
